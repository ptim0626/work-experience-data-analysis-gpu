#!/usr/bin/env python3
"""
GPU brightness adjustment for 2D images.

This script shows the complete implementation of GPU kernels for image
processing using CuPy, with CPU comparison.
"""
import cupy as cp
import numpy as np
...
def gpu_brightness_adjustment(image_stack, brightness_value):
    ...
    brightness_adjust_kernel = cp.RawKernel(kernel_code,
                                            "brightness_adjust_2d")
    ...
    for img_idx in range(num_images):
        ...
        brightness_adjust_kernel(blocks_per_grid,
                                 threads_per_block,
                                 (d_input_stack[img_idx],
                                  d_output_stack[img_idx],
                                  height, width, brightness_value)
                                 )
...
def main():
    ...
    image_stack = simulated_image_stack(num_images=500, height=512, width=512)
