#!/usr/bin/env python3
"""
GPU Gaussian filter for 2D images.

This script shows the complete implementation of GPU kernels for Gaussian
filtering using CuPy, with CPU comparison.
"""
import cupy as cp
import numpy as np
...
def gpu_gaussian_filter(image_stack, sigma=1.0, kernel_size=None):
    ...
    gaussian_filter_kernel = cp.RawKernel(kernel_code, "gaussian_filter_2d")
    ...
    d_output_stack = cp.zeros((num_images, out_height, out_width),
                              dtype=cp.float32)
    d_kernel_weights = cp.asarray(kernel.flatten())
    ...
    for img_idx in range(num_images):
        ...
        gaussian_filter_kernel(blocks_per_grid,
                              threads_per_block,
...
def gpu_gaussian_filter_padded(image_stack, sigma=1.0, kernel_size=None,
    ...
    kernel_code = kernel_file.read_text()
    start_time = time.perf_counter()
    gaussian_filter_kernel = cp.RawKernel(kernel_code,
                                         "gaussian_filter_2d_padded")
    compile_time = time.perf_counter() - start_time
    ...
    for img_idx in range(num_images):
        ...
        gaussian_filter_kernel(blocks_per_grid,
                              ...
                              (d_input_stack[img_idx],
                               d_output_stack[img_idx],
                               d_kernel_weights,
                               height, width, kernel_size, padding_mode)
                              )
    process_time = time.perf_counter() - start_time
